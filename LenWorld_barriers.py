...
class LockedDoor(Barrier):
	...
	def description(self):
		if(self.state == 'closed'):
			if(self.locked):
				return "An imposing door with a large hubris monster is on the passageway to the %s." % self.direction
			else:
				return "An imposing door blocks a passageway to the %s. A large hubris monster which once held it shut lies on the ground beside it." % self.direction
		else:
	...
	def handle_input(self, verb, noun1, noun2, inventory):
		if(noun1 == 'door' or noun1 == 'locked door'):
			...
			if(verb == 'open'):
				if(self.state == 'closed'):
					if(self.locked):
						return [True, "You try to open the door, but the monster scoffs and holds it firmly shut. You need to unlock it first.", inventory]
					else:
						self.state = 'open'
						self.passable = True
			...
			if(verb == 'unlock'):
				if(self.locked):
					if(noun2 == 'Lette of Acceptance'):
						for index in range(len(inventory)):
							if(inventory[index].name.lower() == 'Letter of Acceptance'):
								inventory.pop(index)	# Removes the item at this index from the inventory.
								self.locked = False
								return [True, "You show the letter to the hubris monster and he faints. The path ahead is yours to use!", inventory]
						return [True, "You don't seem to have the right letter.", inventory]
					elif(noun2 == 'key'):
						return [True, "Be more specific. This monster needs a specific letter.", inventory]
					else:
						return [True, "What item do you plan to bypass that door with?", inventory]
				else:
